...
class Schedule(object):
    ...
    def greedySchedule(self):
        ...
        nowPath = Path(currentResource, self.__workPackages, net=self.__net,device=self.__device)
        nowPath.cal_q_score()
        print(nowPath.get_q())
        while True:
            path_list = []
            ...
            values = values.cpu().detach().numpy()[:,0]
            nowPath = path_list[np.argmax(values)]
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('model/model_{}.ckpt'.format(args.model), map_location=torch.device('cpu')))
    model = model.to(device)
    model.eval()
    torch.no_grad()

    workpackages, resources = gen_data.gen_random_data(package_num=define.package_num,seed=0)
    schedule = Schedule(resources, workpackages, None, net=model, time_limit=define.timeLimit, device='cuda:0')
    time0 = time.time()
