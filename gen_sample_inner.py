...
parser.add_argument("--count", type=int, default=3, help='the threshold of random sampling')
...
def gen_random_data(package_num, seed=define.seed):
    workpackages = []
    resources = []
    random.seed(seed)
    tmp = Resource("resource", random.random(), random.random(), timeLimit=define.timeLimit, speed=define.speed)
    resources.append(tmp)
    for i in range(package_num):
        x = random.random()
        y = random.random()
        urgency = random.randint(1,10) * 0.1
...
def f(seed):
    ...
    for i in range(len(path_packages)):
        if random.random() < args.threshold:
            idx_list = list(id2workpackages.keys())
            count = 0
            urgency_list = [0]*define.package_num
            for idx in idx_list:
                wk = id2workpackages[idx]
                tmp_workpackages = copy.deepcopy(id2workpackages)
                tmp_time = total_time - (wk.getWorkingTime() + define.dis(resource_pos[0], wk.getX(), resource_pos[1], wk.getY())/define.speed)
                if tmp_time < 0:
                    continue
                count += 1
                tmp_workpackages.pop(idx, None)
                resources = []
                tmp = Resource("resource", wk.getX(), wk.getY(), timeLimit=tmp_time, speed=define.speed)
                resources.append(tmp)
                packages = list(tmp_workpackages.values())
                schedule = Schedule(resources, packages, replay_memory=None, net=None, is_dqn=False, time_limit=tmp_time, plan_limit=args.limit, time_interval=tmp_time/20)
                schedule.greedySchedule()
                ...
                if len(tmp_path) > 0:
                    ...
                    pid = -1
                tmp_urgency = schedule.get_urgency()
                urgency_list[idx] = tmp_urgency
            wk = path_packages[i]
            data.append([[wk.getX(),wk.getY()], id2workpackages.values(), urgency_list, total_time])
        wk = path_packages[i]
        reward = wk.getUrgency()
    ...
    with lock:
        counter.value += 1
        n_sample.value += len(data)
        if counter.value % 1 == 0:
            sys.stdout.write('\r{}/{}, {}'.format(counter.value, args.number, n_sample.value))
            sys.stdout.flush()
    return data
...
if __name__ == '__main__':
    ...
    if not os.path.exists('sample'):
        os.mkdir('sample')
    pickle.dump(data, open('sample/sample_inner_{}_{}.pkl'.format(args.number, args.threshold), 'wb'))
