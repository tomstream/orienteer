...
class GraphNetDecoder(nn.Module):
    def __init__(self, hidden_size=32, dropout=0.1):
        ...
        self.linear1 = nn.Linear(hidden_size * 3 + 1, hidden_size)
        self.self_attn = transformer.MultiheadAttention(hidden_size, 1, dropout=dropout, my_scaling=False)
    ...
    def forward(self, node_embedding, mask, times, idxc=None):
        ...
        emb = self.linear1(torch.cat([node_avg, f, times.unsqueeze(1), node_last], dim=1))
        emb = emb.unsqueeze(0)
        emb = emb.transpose(0,1)
        weight = torch.sum(self.qlinear(emb) * self.klinear(node_embedding), dim=-1)/math.sqrt(emb.size(-1))
        # weight = 100 * torch.tanh(weight)
...
class Env:
    ...
    def reset(self):
        self.seed = self.range_start + self.count % (self.range_end - self.range_start)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden-size", type=int, default=64, help='generate RL sample or IL sample')
    ...
    for _i in range(max_step):
        ...
        while True:
            ...
            # print(torch.sum(torch.isnan(log_prob)))
